# Changelog

## [0.2.0](https://github.com/mockeroo/mock-responses/compare/mockeroo/mock-responses-v0.1.0...mockeroo/mock-responses-v0.2.0) (2026-02-22)


### Features

* **ruby:** add Ruby gem mockeroo-mock-responses ([#52](https://github.com/mockeroo/mock-responses/issues/52)) ([db538d2](https://github.com/mockeroo/mock-responses/commit/db538d2f894114eb5af549c192d372528edde189))
* trigger release ([689fa6e](https://github.com/mockeroo/mock-responses/commit/689fa6e059c51c20efccff06e68f19f20946f262))
* trigger release ([a581de7](https://github.com/mockeroo/mock-responses/commit/a581de71bd399c5c98d5e020be77986506e9c652))
* trigger release ([85d03c4](https://github.com/mockeroo/mock-responses/commit/85d03c47359dd42405da00a012be24f391707dc5))


### Miscellaneous Chores

* update readme ([#59](https://github.com/mockeroo/mock-responses/issues/59)) ([38e0127](https://github.com/mockeroo/mock-responses/commit/38e0127c898f2bb8e7ae89cee882135c9705dd1f))

## [0.1.0] - 2026-02-21

### Added
- Initial release of the PHP Composer package `mockeroo/mock-responses`.
- `MockResponses::getResponse(int $statusCode)` — returns a random sarcastic response or null.
- `MockResponses::getAvailableCodes()` — returns all supported HTTP status codes.
- Requires PHP >= 8.1.
